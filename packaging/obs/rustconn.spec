...
- UI/UX Enhancements - GNOME HIG compliance improvements:
  - Added accessible labels to connection status icons
  ...
  - Added adw::Clamp to connection dialog for adaptive width
  ...
  - Connection dialog uses adaptive adw::ViewSwitcherTitle with fallback ViewSwitcherBar
- Legacy Code Cleanup - Removed unused TabDisplayMode and TabLabelWidgets types
- Dialog Widget Builders - Created reusable UI components (SwitchRowBuilder, etc.)
- Protocol Dialogs Refactoring - Applied widget builders to SSH, RDP, VNC, SPICE
- Other Dialogs Refactoring - Export, Snippet, Cluster dialogs use widget builders
