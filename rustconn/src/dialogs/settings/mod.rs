//! Settings dialog modules
//!
//! This module contains the settings dialog split into logical components
...
impl SettingsDialog {
    ...
    pub fn new(parent: Option<&Window>) -> Self {
        ...
        ) = create_ssh_agent_tab();

        let clients_tab = create_clients_tab();
        ...
        notebook.append_page(&clients_tab, Some(&Label::new(Some("Clients"))));

        // Wrap notebook in scrolled window for better handling of small screens
    ...
    /// Sets the current settings (for compatibility with existing code)
    pub fn set_settings(&mut self, settings: AppSettings) {
        *self.settings.borrow_mut() = settings;
    }

    /// Shows the dialog and loads current settings (with callback)
        ...
        F: Fn(Option<AppSettings>) + 'static,
    {
        ...
        // Setup save button handler with callback
        let callback_rc = Rc::new(callback);
        self.setup_save_handler_with_callback(callback_rc);
    ...
    /// Sets up the save button handler
    #[allow(dead_code)]
    fn setup_save_handler(&self) {
        // Clone all needed references for the closure
        ...
        let cursor_shape_dropdown_clone = self.cursor_shape_dropdown.clone();
        let cursor_blink_dropdown_clone = self.cursor_blink_dropdown.clone();
    ...
    /// Sets up the save button handler with external callback
    fn setup_save_handler_with_callback(&self, external_callback: Rc<dyn Fn(Option<AppSettings>)>) {
        // Clone all needed references for the closure
        let settings_clone = self.settings.clone();
        let window_clone = self.window.clone();
