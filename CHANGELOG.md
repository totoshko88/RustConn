...
- **`SecretSettings` and `Credentials` derived `Debug`, which would print stored credentials in the clear** — a security audit found both types on `#[derive(Debug)]`. The `SecretString` fields redact themselves, so no runtime password leaked, but `SecretSettings` also holds eight `*_encrypted: Option<String>` fields — a machine-key-encrypted copy of each master password/token/passphrase — and a derived `Debug` printed those ciphertexts verbatim, so anything that formatted the struct into a log or an error carried the encrypted secret with it. Both types now implement `Debug` by hand, showing a `<set>` / `<none>` presence marker for every secret-bearing field (both the `SecretString`s and the `*_encrypted` strings) while leaving non-secret fields readable. The guarantee is enforced by `secret_settings_debug_does_not_leak` and `credentials_debug_does_not_leak`, which format an instance carrying sentinel values and assert none of them appear — so a future plain-`String` secret field added to either struct fails the test rather than silently leaking (M-PUBLIC-DEBUG).

- **Several secret-backend CLI calls had no timeout and could hang the caller forever** — the same audit found that the 1Password (`op`) and Passbolt (`go-passbolt-cli`) backends bounded none of their invocations, and three Bitwarden calls (`login --apikey`, `lock`, `logout`) were also unbounded. Every one of these reaches a remote server or, for `op`, can block on a biometric prompt, so a slow or unreachable host parked the awaiting task indefinitely — the GUI already runs these on worker threads with a status row waiting on them. All of them are now wrapped in `tokio::time::timeout`: 1Password's `run_command`/`run_command_with_stdin` gain an `OP_INVOCATION_TIMEOUT`, Passbolt's `run_command` and its `list user` status probe gain a `PASSBOLT_INVOCATION_TIMEOUT`, and the three Bitwarden calls reuse the existing `BW_INVOCATION_TIMEOUT`. All three constants are 30 seconds — matching the value the rest of the Bitwarden backend already used — and a timeout surfaces as `SecretError::ConnectionFailed` naming the tool and the budget, the same shape the Bitwarden unlock path already produced. Local `--version` availability probes are left unbounded on purpose: they never touch the network.
...
- **Two boxed lists were drawn inside a second border, and an information row carried two icons** — the workspace list sat in a `gtk::Frame` while already carrying the `boxed-list` style class, which draws its own border, so the list appeared with a frame around its frame. The Credentials Storage row in the export dialog had `ℹ` typed into its title *and* a `dialog-information-symbolic` prefix, so the same idea was stated twice — once as a themed icon and once as a character, which does not follow the icon theme, cannot be translated, and is read out by a screen reader as part of the heading.
...
- **Fifteen of sixteen development hooks stopped costing model calls, and the one that reported on the session stopped reporting the wrong session** — `post-session-diagnostics` was an `agent` action on `Stop`, so it started a full agent loop at the end of *every* turn, including turns that only answered a question. It decided what the session had changed by comparing content hashes against a start-of-session baseline, which answers "did *anything* change this file?" — a different question, and wrong whenever the IDE, a second session in the same checkout, or one of the repo's own hooks touches the tree. On 2026-09-06 it fired five times in one session, each time naming 29 `.rs` files the agent had never written, and each time ending in a `getDiagnostics` call that does not exist outside the IDE: in a session driven by `kiro-cli … acp` the tool is absent from the tool set entirely, so the loop could only ever report that it could not check. A new `edit-journal` hook records paths as the write tools are called, which answers the question directly and cannot be fooled by a second writer, and `session-report` scans only those files. Delivery is the interesting half: a `command` hook's stdout is discarded on `Stop`, forwarded only on `SessionStart`, `UserPromptSubmit` and `PreToolUse` — so the scan runs for free at `Stop`, appends to `target/.kiro-session-report`, and `session-report-flush` prints it on the next prompt, a turn that was going to happen anyway. The same channel now carries `flatpak-manifest-check`, which was an agent prompt asking the model to run `test -f` twice and print a fixed warning, and is a timestamp comparison. The three review hooks — `security-review`, `unsafe-review`, `uk-translation-review` — fired on `PostFileSave`, one agent loop per saved file, each seeing a single file in isolation; `uk-translation-review` also ran a full catalogue review after every `msgmerge`, which rewrites `po/uk.po` without changing a single translation. They are replaced by one `commit-review-gate` that asks at `git commit`, driven by the journal, naming the reviewer the change actually needs. `post-task-diagnostics` is the only `agent` action left, and it fires on spec task completion rather than per turn or per file.

- **Every agent profile now declares which model it runs on, because the default was the wrong tier at both ends** — `kiro-cli chat --list-models` reports `default_model: auto`, so all five profiles in `.kiro/agents/` were running on the 1.0x router with the tier chosen per request. `rust-quality-check` runs three fixed cargo commands and clippy contradicts it if it claims a pass it did not earn, so it belongs on the 0.05x tier; `unsafe-reviewer` decides whether a `// SAFETY:` comment is verifiable or merely decorative, nothing repeats that judgement, and a wrong "✅" is undefined behaviour in a shipped binary, so it belongs on the 2.2x one. The rule written down is that the tier follows whether an arbiter exists, not how simple the task looks — which makes this a reallocation rather than a saving: two frequently-invoked agents got cheaper, three audit agents got more expensive than the default. A new `agent-model-guard` hook refuses a profile with no `model:` field and an edit that removes one. It deliberately does not validate the ID, because an unrecognised one falls back to the default with a warning — the same outcome as omitting the field, and the catalogue differs per client, so a profile pinned to an IDE-only ID falls back silently in a CLI session. New steering `cost-discipline.md` holds the policy, the tier table and the four hook patterns that came out of this.

- **A finished feature now gets a changelog entry and a commit, and nothing gets pushed** — the commit is staged from the edit journal rather than with `git add -A`, because this checkout is regularly shared with the IDE and a second session: while these very changes were being made it held 47 dirty files, 29 of them `.rs`, none of them the agent's, and `HEAD` moved underneath the session. `release-manual-only-guard` now refuses `git push` outright — any remote, any ref, branch or tag — where it previously refused only a push carrying a version tag. Widening it exposed a matching flaw: neither `push` nor `tag` was anchored to command position, so under a blanket ban `echo "then git push it"` would have been refused; both are anchored now, and `git push --dry-run`, `git commit` and tag listing and deletion stay allowed. `commit-message-helper.md` says explicitly that it generates text and does not commit, so it no longer reads as a contradiction of the rule.

- **`shell-environment.md` was split into rules and reasoning, cutting the always-loaded context by 470 words** — every `inclusion: always` steering file is a fixed token cost on every request in every session, and at 1 691 words this one was spending most of that on explaining itself: the queued-`sleep` incident, cargo timings, the `/tmp`-to-`target/` correction, the hour spent diagnosing a portal-blocked file chooser. The rules stay `always` at 890 words; the evidence moved to `shell-environment-why.md` (`inclusion: manual`), where it is more useful because it is read when a rule looks arbitrary rather than on every turn. No rule lives only in the companion. `kirograph.md` was examined for the same treatment and left alone — it is a question-to-tool table and four actionable gotchas, with nothing to move. Also added `scripts/change-inventory.sh`, which writes a deterministic inventory of a change set (diffstat, touched crates, tests in them, which specialised reviews apply, whether the clippy log can be trusted) so a fresh review session does not spend its first tokens rediscovering a diff that a shell command already knows.
