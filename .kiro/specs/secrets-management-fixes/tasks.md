...
- [ ] 1. Виправлення 5+2: Розшифрування пароля Bitwarden та auto-unlock при старті
  - [ ] 1.1 Додати розшифрування `bitwarden_password_encrypted` в `AppState::new()`
  ...
  - [ ] 1.2 Додати eager auto_unlock Bitwarden в `AppState::new()`
  ...
  - [ ] 1.3 Написати property тест для round-trip шифрування/розшифрування пароля Bitwarden
...
- [ ] 2. Checkpoint — Перевірити що тести проходять
...
- [ ] 3. Виправлення 1: Диспетчеризація бекенду при Load from Vault для груп
  - [ ] 3.1 Замінити захардкоджений LibSecretBackend в `connection_dialogs.rs`
...
- [ ] 4. Виправлення 3: Узгодження ключів для Inherit з не-KeePass бекендами
  - [ ] 4.1 Додати обробку Inherit для не-KeePass бекендів в `resolve_credentials_blocking`
  ...
  - [ ] 4.2 Перевірити що `save_group_password_to_vault` використовує `group.id.to_string()` як lookup_key
  ...
  - [ ] 4.3 Написати property тест для узгодженості lookup_key
  ...
  - [ ] 4.4 Написати property тест для обходу ієрархії при Inherit
  ...
  - [ ] 4.5 Написати property тест для merge облікових даних групи
...
- [ ] 5. Checkpoint — Перевірити що тести проходять
  - Запустити `cargo test -p rustconn-core --test property_tests secret_fixes` та `cargo clippy --all-targets`
  - Переконатися що всі тести проходять, запитати користувача якщо є питання.

- [ ] 6. Виправлення 4: Автоматичне збереження пароля при підключенні з Vault
  - [ ] 6.1 Додати auto-save для RDP при password_source == Vault
  ...
  - [ ] 6.2 Додати auto-save для VNC при password_source == Vault
    - У файлі `rustconn/src/window/rdp_vnc.rs`, VNC password callback (~рядок 678)
    - Аналогічна зміна як для RDP
    - _Requirements: 4.1, 4.4_

- [ ] 7. Виправлення 6: Toast сповіщення при помилках збереження
  - [ ] 7.1 Додати Toast callback у `save_password_to_vault` та `save_group_password_to_vault`
...
- [ ] 8. Фінальний checkpoint — Перевірити що всі тести проходять
