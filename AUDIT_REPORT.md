...
- [ ] **SEC-01: Плейсхолдери SHA256 у cli_download.rs**
...
- [ ] **SEC-02: Ін'єкція через змінні у командах**
...
- [ ] **SEC-03: Логування повної команди в stderr (CLI)**
...
- [ ] **SEC-04: Документ-шифрування — фіксовані параметри Argon2**
...
- [ ] **CODE-01: Монолітний CLI — 5000+ рядків в одному main.rs**
...
- [ ] **CODE-02: `--config` прапорець оголошений але не використовується**
...
- [ ] **CODE-03: Дублювання build_command() між CLI та core**
...
- [ ] **CODE-04: Дублювання VNC viewer detection**
...
- [ ] **CODE-05: Дублювання icon mapping**
...
- [ ] **GUI-01: Деякі діалоги використовують `gtk4::Window` замість `adw::Window`**
...
- [ ] **GUI-03: Sidebar мінімальна ширина 200px — забагато для телефонів**
...
- [ ] **GUI-04: Відсутні accessible names для icon-only кнопок**
...
- [ ] **GUI-05: Валідація форм не анонсується screen readers**
  ...
  if !is_valid {
      ...
      // Також встановити описову помилку
...
- [ ] **EXT-01: Flatpak не може запускати зовнішні клієнти, але UI пропонує їх завантажити**
...
- [ ] **EXT-02: Hardcoded версії та URL у DOWNLOADABLE_COMPONENTS**
...
| Flathub verification | ❓ | Потрібно перевірити на flathub.org |

### 🟡 P1 — Покращення

- [ ] **FH-01: Уніфікувати SPDX ліцензію**
...
- [ ] **FH-02: Додати `<translation>` елемент**
  - Flathub рекомендує вказувати систему перекладу
  - Зараз i18n не реалізовано
  - **Рішення (довгострокове):** Додати gettext або fluent:
  ```xml
  <!-- Коли i18n буде реалізовано: -->
  <translation type="gettext">rustconn</translation>
  ```

- [ ] **FH-03: Додати `<recommends>` та `<requires>`**
...
- [ ] **CLI-01: Підключити `--config` або видалити**
...
- [ ] **CLI-03: Додати `--no-color` та `NO_COLOR` env**
...
- [ ] **CLI-04: Додати `--dry-run` для `connect`**
...
- [ ] **CLI-05: Додати shell completions**
...
- [ ] **CLI-06: Pager для довгого виводу**
...
## 7. Питання для уточнення

1. **Flatpak extensions vs embedded-only:** Чи планується підтримка зовнішніх CLI у Flatpak через extensions, чи стратегія — тільки embedded клієнти?

2. **i18n:** Чи планується локалізація? Якщо так — gettext чи fluent? Це впливає на metainfo (`<translation>`) та структуру рядків у коді.

3. **Мобільна підтримка:** Чи є плани на підтримку Phosh/GNOME Mobile? Якщо так — потрібна серйозніша адаптація (NavigationView замість OverlaySplitView для деяких flow).

4. **CLI модуляризація:** Чи є бажання розбити `main.rs` на модулі зараз, чи це low priority?

5. **FreeRDP 2.x vs 3.x:** Чи потрібна підтримка обох версій? Аргументи CLI відрізняються (`/v:` vs `--server`). Зараз detection знаходить обидві, але `build_rdp_command()` генерує тільки FreeRDP 2.x формат.

6. **Snap packaging:** Чи активно підтримується snap? `snap.rs` модуль є, але snap manifest не знайдений у репозиторії.

7. **Flathub verification:** Чи пройдена верифікація на Flathub? Це впливає на видимість та довіру.

8. **Property tests coverage:** Зараз 1241 тест. Чи є цільовий показник покриття? Які модулі потребують додаткових тестів?
...
|----|-----------|-----------|------|
| SEC-01 | Безпека | 🔴 P0 | Плейсхолдери SHA256 у cli_download.rs |
| SEC-02 | Безпека | 🔴 P0 | Ін'єкція через змінні у командах |
| EXT-01 | Компоненти | 🔴 P0 | Flatpak пропонує завантажити непрацюючі CLI |
| EXT-02 | Компоненти | 🔴 P0 | Hardcoded версії без auto-update |
...
| GUI-01 | GUI | 🟡 P1 | gtk4::Window замість adw::Window |
| GUI-02 | GUI | 🟡 P1 | Фільтри переповнюють на мобільних |
| GUI-03 | GUI | 🟡 P1 | Sidebar 200px — забагато для телефонів |
...
|---------|--------|----------|
| Безпека | 7/10 | Сильні паттерни (SecretString, thiserror, ring), але checksum gaps та variable injection |
| Якість коду | 7/10 | Чистий Rust, strict clippy, але монолітний CLI та дублювання |
| GNOME HIG | 9/10 | Відмінне використання libadwaita, breakpoints, адаптивність |
| Мобільна підтримка | 7/10 | Базова адаптивність є, потрібні breakpoints для <400sp |
| Зовнішні компоненти | 5/10 | Hardcoded версії, placeholder checksums, Flatpak обмеження |
| Flathub | 8/10 | Майже повна відповідність, дрібні покращення |
| CLI (clig.dev) | 6/10 | Хороша структура команд, але відсутні --verbose, --color, completions |
| Wayland | 9/10 | Wayland-first, немає X11 API, Cairo fallback |
| Тестування | 8/10 | 1241 property test, proptest, tempfile |
