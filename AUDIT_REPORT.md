...
- [x] **GUI-07: Tray polling кожні 250ms** ✅ v0.8.7
  - Розділено на два потоки: повідомлення від tray перевіряються кожні 50ms (легкий `try_recv`), стан (сесії, з'єднання) синхронізується кожні 2 секунди з dirty-flag tracking
  - Зменшено навантаження CPU у idle: замість одного 250ms циклу з обробкою повідомлень + оновленням стану, тепер окремі цикли з оптимальними інтервалами
...
- [x] **GUI-09: Drag-and-drop недоступний для keyboard-only** ✅ v0.8.7
  - Додано `Ctrl+M` shortcut для "Move to Group" діалогу — повноцінна клавіатурна альтернатива drag-and-drop

- [x] **GUI-10: Навігація по історії пошуку** ✅ v0.8.7
  - Up/Down стрілки циклюють через історію пошуку inline у search entry
  - Escape повертає оригінальний текст і закриває popover
  - Будь-яка інша клавіша скидає навігацію
...
- [x] **EXT-07: Автоматизація оновлення версій через CI** ✅ v0.8.7
  - GitHub Action `.github/workflows/check-cli-versions.yml` запускається щопонеділка
  - Перевіряє latest версії kubectl, Tailscale, Cloudflared, Boundary, Teleport, Bitwarden CLI, 1Password CLI
  - Показує pinned версії з `cli_download.rs` для порівняння
...
- [x] **EXT-08: Кешування результатів client detection** ✅ v0.8.7
  - `ClientDetectionResult::detect_cached()` повертає кешований результат якщо він молодший 5 хвилин
  - Використовує `OnceLock<Mutex<Option<(Result, Instant)>>>` — thread-safe, poison-resistant
...
- [x] **FH-06: Додати `x-checker-data` для всіх modules у Flathub manifest** ✅ v0.8.7
...
**Виконано:** 41/46 задач (89%) — всі P0, майже всі P1 та P2
**Залишилось:** 5 задач (SEC-05, GUI-02/06-10, FH-04-07)
